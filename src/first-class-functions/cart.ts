...
export const object_remove = (obj: { [k: string]: any }, k: string) => {
  const copy = Object.assign({}, obj);
  delete copy[k];
  return copy;
};
...
export const set_field = (item: TCart, field: TField, value: any) =>
  object_set(item, field, value);
...
export const set_field_by_name = <T>(
  ...
  field: keyof TCart,
  value: T
) => object_set(cart, name, set_field(cart[name], field, value));
